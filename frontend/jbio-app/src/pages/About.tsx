...
const currentDevs = allProfiles.filter((p) => p.role === 'Developer' && p.current !== false);
const pastDevs = allProfiles.filter((p) => p.role === 'Developer' && p.current === false);
    ...
    <div className='about-page'>
      <div className='about-intro'>
        <aside className='about-sponsor'>
          ...
          <div className='sponsor-list'>
            {sponsors.map((s) => (
              <Profile key={s.name} {...{ ...s, photo: resolvePhoto(s.name) }} size='large' />
            ))}
          </div>
        </aside>
      ...
      <section className='dev-team'>
        ...
        {coaches.length > 0 && (
          <div className='team-section'>
            ...
            <div className='dev-grid'>
              {coaches.map((c) => (
                <Profile key={c.name} {...{ ...c, photo: resolvePhoto(c.name) }} size='small' about='🎬 Coach' />
              ))}
            </div>
          </div>
        )}
        ...
        <div className='team-section'>
          ...
          <div className='dev-grid'>
            {currentDevs.map((d) => (
              <Profile key={d.name} {...{ ...d, photo: resolvePhoto(d.name) }} size='small' about='⚡ Active Developer' />
            ))}
          </div>
        </div>
        ...
        <div className='team-section'>
          ...
          <div className='dev-grid'>
            {pastDevs.map((d) => (
              <Profile key={d.name} {...{ ...d, photo: resolvePhoto(d.name) }} size='small' about='↩️ Previous Contributor' />
            ))}
          </div>
        </div>
      </section>
